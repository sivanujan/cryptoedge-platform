...
async def ws_prices(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            try:
                from services.binance_service import get_ticker_info
                # Run sync CCXT calls in a threadpool to avoid blocking the event loop
                btc = await asyncio.to_thread(get_ticker_info, "BTC/USDT")
                eth = await asyncio.to_thread(get_ticker_info, "ETH/USDT")
        ...
        logger.info("WebSocket price client disconnected or cancelled")
    finally:
...
async def ws_signals(websocket: WebSocket):
    ...
    try:
        ...
        logger.info("WebSocket signal client disconnected or cancelled")
    finally:
